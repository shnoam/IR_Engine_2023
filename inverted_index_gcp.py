...
class InvertedIndex:
    def __init__(self, docs={}):
        ...
        self.document_len = defaultdict(list)
    ...
    def read_posting_list(self, directory,w):       # added : directory
        with closing(MultiFileReader()) as reader:
            posting_list = []
            if w in self.posting_locs:
                locs = self.posting_locs[w]
                b = reader.read(directory,locs, self.df[w] * TUPLE_SIZE)        # added : directory
    ...
    def posting_lists_iter(self, directory, query):
    ...
    @staticmethod
    def write_a_posting_list(b_w_pl,bucket_name,stor_path):
        ...
        with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl:
                ...
                b = b''.join([(doc_id << 16 | (tf & TF_MASK)).to_bytes(TUPLE_SIZE, 'big')
                              for doc_id, tf in pl])
                # write to file(s)
                locs = writer.write(b, stor_path)
                # save file locations to index
                posting_locs[w].extend(locs)
            writer.upload_to_gcp(stor_path)
            InvertedIndex._upload_posting_locs(bucket_id, posting_locs, bucket_name, stor_path)
        return bucket_id
