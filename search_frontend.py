...
def get_content_from_storage(bucket, file_name):
    blob = storage.Blob(f'{file_name}', bucket)  # todo - change the directory name pr
...
def cosine_similarity(D, Q, index):  # for search_body
    ...
    for i, docid in enumerate(D.index):
        cosine_sim_dic[docid] = all_docs_cosine_sim[i] / (
        index.doc_id_to_norm[docid])  # numerator[i] /( index.doc_id_to_norm[doc_id] * norma_Q)
...
def search():
    ...
    if len(tokenized_query)==1 :
        final_docs_scores = sorted([(doc_id, score) for doc_id, score in title_score_values.items()], key=lambda x: x[1],
                                   reverse=True)[:50]  # 100
    ...
    words_anchor, pls_anchor = zip(*inverted_index_anchor.posting_lists_iter('postings_gcp/anchor_index',
                                                                             tokenized_query))  # words = all words in the titles,pls = list of psl (each psl = tups of doc_id and freq)
    final_docs_scores = defaultdict(int)
    ...
    for doc_id, score in title_score_values.items():
...
def search_body():
    ...
    words_body, pls_body = zip(*inverted_index_text.posting_lists_iter('postings_gcp/text_index',
                                                                       tokenized_query))  # words = all words in the titles,pls = list of psl (each psl = tups of doc_id and freq)
    sorted_candidate_doc = get_topN_for_query(tokenized_query, inverted_index_text, words_body, pls_body)
...
def search_title():
    ...
    # BEGIN SOLUTION
    tokenized_query = tokenize(query)
    words_title, pls_title = zip(*inverted_index_title.posting_lists_iter('postings_gcp/title_index',
                                                                          tokenized_query))  # words = all words in the titles,pls = list of psl (each psl = tups of doc_id and freq)
    sorted_candidate_doc = get_candidate_documents_tf(tokenized_query, False, words_title,
                                                      pls_title)  # TODO : cehck if get_candidate_documents_binaryScores
    for doc, tf_term in sorted_candidate_doc:
...
def search_anchor():
    ...
    words_anchor, pls_anchor = zip(*inverted_index_anchor.posting_lists_iter('postings_gcp/anchor_index',
                                                                             tokenized_query))  # words = all words in the titles,pls = list of psl (each psl = tups of doc_id and freq)
    sorted_candidate_doc = get_candidate_documents_tf(tokenized_query, False, words_anchor,
                                                      pls_anchor)  # TODO : cehck if get_candidate_documents_binaryScores
    for doc, tf_term in sorted_candidate_doc:
